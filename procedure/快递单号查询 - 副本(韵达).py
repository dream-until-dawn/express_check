    ...
    for danghao in danghao_list:
        ...
        try:
            if user_dict['data']['info']['status'] == '1':
                ...
                excel_data.append({'danhao':danghao,'time':str_time,'message':user_dict['data']['info']['context'][0]['desc']})
                row_count += 1
...
def excel_new():
    ...
    for user in excel_data:
        ...
        sheet.cell(row_count,2,user['time'])
        sheet.cell(row_count,3,user['message'])
        row_count += 1
    str_time = time.localtime(int(time.time()))
